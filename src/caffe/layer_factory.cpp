...
shared_ptr<Layer<Dtype> > GetConvolutionLayer(const LayerParameter &param)
{
  ...
  if(engine==ConvolutionParameter_Engine_CAFFE) {
    ...
    if(use_dilation) {
      LOG(FATAL) << "CuDNN doesn't support the dilated convolution at Layer " << param.name();
    }
...
shared_ptr<Layer<Dtype> > GetPoolingLayer(const LayerParameter &param)
{
  ...
  if(engine==PoolingParameter_Engine_CAFFE) {
    ...
    if(param.top_size()>1) {
      LOG(INFO) << "cuDNN does not support multiple tops. Using Caffe's own pooling layer.";
      return shared_ptr<Layer<Dtype> >(new PoolingLayer<Dtype>(param));
    }
