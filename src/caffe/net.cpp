...
void Net<Dtype>::Init(const NetParameter& in_param)
{
  ...
  for(int layer_id = 0; layer_id < param.layer_size(); ++layer_id) {
    ...
    if(layer_param.propagate_down_size() > 0) {
      CHECK_EQ(layer_param.propagate_down_size(), layer_param.bottom_size()) << "propagate_down param must be specified either 0 or bottom_size times ";
    }
    ...
    if(layer->AutoTopBlobs()) {
      ...
      for(; num_top < needed_num_top; ++num_top) {
        ...
        // blob_name_to_idx as we don't want these blobs to be usable as input to other layers.
  ...
  for(int layer_id = layers_.size() - 1; layer_id >= 0; --layer_id) {
    ...
    for(int top_id = 0; top_id < top_vecs_[layer_id].size(); ++top_id) {
      ...
      if(blobs_skip_backp.find(blob_name) == blobs_skip_backp.end()) layer_skip_propagate_down = false;
...
void Net<Dtype>::ToHDF5(const string &filename, bool write_diff) const
{
  ...
  for (int layer_id = 0; layer_id < layers_.size(); ++layer_id) {
    ...
    hid_t layer_data_hid = H5Gcreate2(data_hid, layer_name.c_str(), H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    CHECK_GE(layer_data_hid, 0) << "Error saving weights to " << filename << ".";
    hid_t layer_diff_hid = -1;
    if(write_diff) {
      layer_diff_hid = H5Gcreate2(diff_hid, layer_name.c_str(), H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
